...
import (
	...
	"devops/core"
	"devops/core/util/gofile"
	"devops/core/util/command"
...
func hookCommandTaskRun(s string, applyId int) {
	scriptFile := gostring.JoinStrings(core.App.LocalTmpSpace, "/", gostring.StrRandom(24), ".sh")
	if err := gofile.CreateFile(scriptFile, []byte(s), 0744); err != nil {
		core.App.Logger.Error("hook script file create failed, err[%s], apply_id[%d]", err.Error(), applyId)
        return
	}
	cmds := []string{
	...
	if err := task.GetError(); err != nil {
		core.App.Logger.Error("hook script run failed, err[%s], output[%s], apply_id[%d]", err.Error(), gostring.JsonEncode(task.Result()), applyId)
    }
}